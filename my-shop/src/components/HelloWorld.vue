...
export default {
  ...
  created () {
    ...
    console.log(this.goods_obj, '========2')
    console.log(this.goods_length, '========3')
    console.log(this.$store.state.shop, '=========4') // 模块化 Vuex允许我们将 store 分割成模块（module）每个模块拥有自己的 state、mutation、action、getter、
  },
  computed: {
    ...mapState([
  ...
  methods: {
    ...
    reduce_num (id) {
      // this.REDUCE_CART(id) // mapMutation方法的REDUCE_CART要放开
      this.$store.commit('REDUCE_CART', id) // 也可以直接获取
    },
    add_num (id) {
      this.ADD_CART(id) // 通过mapMutations 辅助函数获取
      // this.$store.commit('ADD_CART', id)  // 也可以直接获取
    }
  }
}
</script>
